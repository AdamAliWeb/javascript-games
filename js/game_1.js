...
let switch_player = false, two_players = null,
...
const check_winners = (win_choices, player) => {
  ...
  win_choices.forEach(nums => {
    ...
    for (let i = 0; i < player.length; i++) {
      nums.forEach(win_choice => win_choice === player[i] ? check_line++ : null);
    }
...
const disable_events = (option) => {
  $game1_btns.forEach(btn => option ? btn.classList.add("game-available") : btn.classList.remove("game-available"));
}
...
const game1_start = (e, btns, player1, player2) => {
  ...
  if (!switch_player) {
    two_players ? $game1_btns.forEach(btn => btn.classList.toggle("player2")) : null;
    insert_sign(btn_index, player1, player1_choices, true);

    if (!two_players && !check_draw() && !check_winners(winners, player1_choices)) {
      disable_events(true);
      ...
      setTimeout(() => {
        ...
        if (check_winners(winners, player2_choices)) {
          end_message("Jugador 2 gano la partida");
        }
      }, 1000);
    }

    if (check_winners(winners, player1_choices)) {
      end_message("Jugador 1 gano la partida");
    } else if (check_draw()) {
      end_message("Empate");
    }
...
export const game1_function = () => {
  d.addEventListener("click", (e) => {
    ...
    e.target.matches(".game-1-btn") ? game1_start(e, $game1_btns, "x", "o") : null;
